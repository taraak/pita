...
#SBATCH --array=0-2  # Adjust the range to match number of sigma_min values

# Define different sigma_min values
SIGMA_MIN_LIST=(0.01 0.05 0.1)
...
pip install --no-index -r cc_requirements_tara.txt
cd runner/
pip install -e .
...
tags=["test","ALDP"] \
model.noise_schedule.sigma_min=$SIGMA_MIN \
