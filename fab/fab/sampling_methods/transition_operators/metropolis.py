...
class Metropolis(TransitionOperator):
    def __init__(
        ...
        self.register_buffer(
            "noise_scalings",
            torch.linspace(max_step_size, min_step_size, n_updates).repeat(
    ...
    def transition(self, point: Point, i: int, beta: float) -> Point:
        ...
        for n in range(self.n_updates):
            ...
            # acceptance
            acceptance_probability = torch.nan_to_num(
                acceptance_probability, nan=0.0, posinf=0.0, neginf=0.0
            )
            accept = (
                acceptance_probability > torch.rand(acceptance_probability.shape).to(x.device)
            ).int()
