...
from fab.sampling_methods import (
...
def analytic_alpha_2_div(mean_q: torch.Tensor, mean_p: torch.Tensor) -> torch.Tensor:
    r"""Calculate \int p(x)^2/q dx where p and q are unit-variance Gaussians."""
    return torch.exp(torch.sum(mean_p**2 + mean_q**2 - 2 * mean_p * mean_q))


def test_ais_num_dist(
    dim: int = 1,
    ...
    p_target = False
    logger = ListLogger()
    base_dist = WrappedTorchDist(
        torch.distributions.MultivariateNormal(
            loc=torch.zeros(dim) + 0.5, scale_tril=torch.eye(dim)
        )
    )
    target = WrappedTorchDist(
        torch.distributions.MultivariateNormal(
            loc=torch.zeros(dim) - 0.5, scale_tril=torch.eye(dim)
        )
    )
    ...
    for n_ais_intermediate_distributions in n_intermedaite_distributions_list:
        ...
        x_linspace = torch.linspace(-5.0, 5.0, 50)[:, None]
    ...
    plt.plot(
        logger.history["n_ais_intermediate_distributions"],
        np.abs(logger.history["log_Z_estimate"]),
        "o",
...
def setup_ais(
    ...
    base_dist = WrappedTorchDist(
        torch.distributions.MultivariateNormal(loc=torch.zeros(dim), scale_tril=3 * torch.eye(dim))
    )
...
def test_ais__eval_batch(
    ...
    base_samples, base_log_w, ais_samples, ais_log_w = ais.generate_eval_data(
...
def test_ais__overall(
    ...
    fig, axs = plt.subplots(
        n_plots_total, 2, figsize=(2 * 3, n_plots_total * 3), sharex=True, sharey=True
    )
    ...
    for i in range(n_iterations):
        ...
        if i in plotting_iterations:
            ...
            axs[plot_index, 1].plot(resampled_points[:, 0], resampled_points[:, 1], "o", alpha=0.5)
