...
class ReplayDataModule(torch.nn.Module):
    def __init__(
        ...
        self.register_buffer("current_index", current_index)
...
class SimpleBuffer(torch.nn.Module):
    def __init__(
        ...
        self.is_full = False  # whether the buffer is full
        self.can_sample = True  # whether the buffer is full enough to begin sampling
        self.sample_with_replacement = sample_with_replacement
        self.prioritize = prioritize
    ...
    def init_generator(self, seed=None):
        """Initialise the generator with a seed."""
        self.generator.manual_seed(seed)

    @property
    def current_index(self):
        return self.buffer.current_index.item()
    ...
    def add(
        ...
        if not self.is_full:
            self.is_full = new_index >= self.max_length
            self.can_sample = new_index >= self.min_sample_length
        self.buffer.current_index = torch.tensor(new_index % self.max_length).to(x.device)
    ...
    def sample(
        ...
        if self.sample_with_replacement:
            if prioritize:
                ...
                ).sample((batch_size,), generator=self.generator)
            else:
                indices = torch.randint(max_index, (batch_size,), generator=self.generator)
        else:
            if prioritize:
                raise NotImplementedError(
                    "Prioritised sampling without replacement is not implemented with generator"
                )
                indices = sample_without_replacement(self.buffer.energy[:max_index], batch_size)
            else:
                indices = torch.randperm(max_index, generator=self.generator)[:batch_size]
    ...
    def save(self, path):
        ...
        to_save = {
            ...
            "current_index": self.buffer.current_index.detach().cpu(),
            "is_full": self.is_full,
            "can_sample": self.can_sample,
        }
        torch.save(to_save, path)
    ...
    def load(self, path):
        ...
        self.buffer.current_index = torch.tensor(old_buffer["current_index"]).to(self.device)
        self.is_full = old_buffer["is_full"]
        self.can_sample = old_buffer["can_sample"]
