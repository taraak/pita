...
class DIT3D(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(
        ...
        n_particles,
        vocab_size: int,
        condition_on_temperature: bool = True,
    ):
        super().__init__()
        config = omegaconf.OmegaConf.create()
        config.model = omegaconf.OmegaConf.create()
        ...
        self.condition_on_temperature = condition_on_temperature
    ...
    def forward(self, t, x, inverse_temp=None):
        ...
        if self.condition_on_temperature:
