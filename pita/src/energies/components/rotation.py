...
class Random3DRotationTransform(torch.nn.Module):
    def __init__(self, num_particles, n_spatial_dim):
        super().__init__()
    ...
    def forward(self, data, force=None):
        data = data.reshape(
            -1, self.num_particles, self.num_dimensions
        )  # batch dimension needed for einsum
        rot = self.get_rot_like(data)
        data = torch.einsum("bij,bki->bkj", rot, data)
        data = data.reshape(
            -1, self.num_particles * self.num_dimensions
        )  # don't want to return with batch dim

        if force is not None:
            force = force.reshape(-1, self.num_particles, self.num_dimensions)
