...
def sample_from_tensor(x, n_samples):
    """
    Sample n_samples from a tensor x.
    Args:
    ...
    """
    indices = torch.randint(0, x.shape[0], (n_samples,))
    return x[indices]
...
class BaseEnergyFunction(ABC):
    ...
    def sample_test_set(self, num_points: int, normalize: bool = False) -> Optional[torch.Tensor]:
        ...
        outs = sample_from_tensor(self.test_set, num_points)
        if normalize:
    ...
    def sample_train_set(self, num_points: int, normalize: bool = False) -> Optional[torch.Tensor]:
        ...
        outs = sample_from_tensor(self.train_set, num_points)
        if normalize:
            outs = self.normalize(outs)
    ...
    def sample_val_set(self, num_points: int, normalize: bool = False) -> Optional[torch.Tensor]:
        ...
        outs = sample_from_tensor(self.val_set, num_points)
        if normalize:
            outs = self.normalize(outs)
