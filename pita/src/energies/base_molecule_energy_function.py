...
class BaseMoleculeEnergy(BaseEnergyFunction):
    def __init__(
        ...
        # If LJ potential is used, use 1 decimal place
        if "LJ" in data_name:
            temp_format = "{:0.1f}"
        else:
            temp_format = "{:0.2f}"
        ...
        self.data_path_train = (
            data_path
            + f"{data_name}{self.n_particles}_temp_{temp_format.format(self.temperature)}/train_split_{data_name}{self.n_particles}-10000.npy"
            # + f"{data_name}{self.n_particles}_temp_{self.temperature:0.2f}/train_split_{data_name}{self.n_particles}-10000.npy"
        )
        self.data_path_val = (
            data_path
            + f"{data_name}{self.n_particles}_temp_{temp_format.format(self.temperature)}/val_split_{data_name}{self.n_particles}-10000.npy"
            # + f"{data_name}{self.n_particles}_temp_{self.temperature:0.2f}/val_split_{data_name}{self.n_particles}-10000.npy"
        )
        self.data_path_test = (
            data_path
            + f"{data_name}{self.n_particles}_temp_{temp_format.format(self.temperature)}/test_split_{data_name}{self.n_particles}-10000.npy"
    ...
    def setup_test_set(self):
        ...
        data = self.maybe_normalize(data)
