...
for peptide in tqdm.tqdm(training_peptides):
    ...
    h_dict[peptide] = torch.nn.functional.one_hot(
        torch.tensor(atom_types_dict[peptide]), num_classes=5
    )
...
for peptide in tqdm.tqdm(validation_peptides):
    ...
    val_h_dict[peptide] = torch.nn.functional.one_hot(
        torch.tensor(val_atom_types_dict[peptide]), num_classes=5
    )
...
def resample_noise(
    ...
    for peptide in peptides:
        ...
        x0 = priors[peptide].sample(n_batch*n_samples).cpu().reshape(n_batch,n_samples, -1)
        x0 = torch.nn.functional.pad(x0, (0, (max_atom_number - n_particles) * 3))

        noise = priors[peptide].sample(n_batch*n_samples).cpu().reshape(n_batch,n_samples, -1)
...
for peptide in validation_peptides:
    ...
    h = (
        torch.cat(
            [val_h_dict[peptide], torch.zeros(max_atom_number - n_particles, 5)]
        )
        .unsqueeze(0)
...
for peptide in training_peptides:
    ...
    h = (
        torch.cat(
            [h_dict[peptide], torch.zeros(max_atom_number - n_particles, 5)]
        )
        .unsqueeze(0)
        .repeat(n_batch, 1, 1)
    )
