...
class BufferTrainer:
    ...
    def run(
        ...
        for i in pbar:
            ...
            x_ais, log_w_ais = self.model.annealed_importance_sampler.sample_and_log_weights(
                batch_size
            )
            x_ais = x_ais.detach()
            ...
            info.update(
                loss=loss.cpu().detach().item(), step=i, grad_norm=grad_norm.cpu().detach().item()
            )
            ...
            if n_checkpoints is not None:
                if i in checkpoint_iter:
                    ...
                    torch.save(
                        self.optimizer.state_dict(), os.path.join(checkpoint_path, "optimizer.pt")
                    )
                    if self.optim_schedular:
                        torch.save(
