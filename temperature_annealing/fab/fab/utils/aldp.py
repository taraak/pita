...
def evaluate_aldp(
    z_sample, z_test, log_prob, transform, iter, metric_dir=None, plot_dir=None, batch_size=1000
):
    """
    ...
    if isinstance(transform.transform, bg.mixed.MixedTransform):
        ...
        raise NotImplementedError("The evaluation is not implemented " "for this transform.")
    ...
    kld_phi = np.sum(htest_phi * np.log((htest_phi + eps) / (hgen_phi + eps))) * 2 * np.pi / nbins
    ...
    kld_psi = np.sum(htest_psi * np.log((htest_psi + eps) / (hgen_psi + eps))) * 2 * np.pi / nbins
    ...
    kld_ram = (
        np.sum(hist_ram_test * np.log((hist_ram_test + eps_ram) / (hist_ram_gen + eps_ram)))
        * (2 * np.pi / nbins_ram) ** 2
    )
    ...
    if metric_dir is not None:
        ...
        if os.path.exists(kld_path):
            ...
            kld_hist = kld_append
        np.savetxt(
            kld_path, kld_hist, delimiter=",", header="it,kld_phi,kld_psi,kld_ram", comments=""
        )
    ...
    if plot_dir is not None:
        ...
        if transform_mode == "internal":
            hists_test_bond = np.concatenate((hists_test_cart[:, :2], hists_test_bond), 1)
            hists_gen_bond = np.concatenate((hists_gen_cart[:, :2], hists_gen_bond), 1)
            hists_test_angle = np.concatenate((hists_test_cart[:, 2:], hists_test_angle), 1)
            hists_gen_angle = np.concatenate((hists_gen_cart[:, 2:], hists_gen_angle), 1)
        ...
        for i in range(len(label)):
            ...
            plt.savefig(
                os.path.join(plot_dir, "marginals_%s_%07i.png" % (label[i], iter + 1)), dpi=300
            )
            plt.close()
        ...
        plt.savefig(os.path.join(plot_dir, "phi_psi_%07i.png" % (iter + 1)), dpi=300)
        plt.close()

        # Ramachandran plot
        plt.figure(figsize=(10, 10))
        plt.hist2d(
            phi, psi, bins=64, norm=mpl.colors.LogNorm(), range=[[-np.pi, np.pi], [-np.pi, np.pi]]
        )
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.xlabel(r"$\phi$", fontsize=24)
        plt.ylabel(r"$\psi$", fontsize=24)
        plt.savefig(os.path.join(plot_dir, "ramachandran_%07i.png" % (iter + 1)), dpi=300)
        plt.close()
