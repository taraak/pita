...
class ManyWellEnergy(DoubleWellEnergy, TargetDistribution):
    ...
    def __init__(
        self, dim=4, use_gpu: bool = True, normalised: bool = False, a=-0.5, b=-6.0, c=1.0
    ):
        assert dim % 2 == 0
        self.n_wells = dim // 2
        super().__init__(dim=2, a=a, b=b, c=c)
        self.dim = dim
        self.centre = 1.7
    ...
    def sample(self, shape):
        ...
        return torch.concat(
            [super(ManyWellEnergy, self).sample(shape) for _ in range(self.n_wells)], dim=-1
        )
    ...
    def get_modes_test_set_iterator(self, batch_size: int):
        ...
        if self.dim < self.max_dim_for_all_modes:
            ...
            test_set[
                :, torch.arange(self.dim) % 2 == 0
            ] = -self.centre + self.centre * 2 * torch.randint(
                high=2, size=(outer_batch_size, int(self.dim / 2))
            )
        return DatasetIterator(batch_size=batch_size, dataset=test_set, device=self.device)
    ...
    def performance_metrics(
        ...
        if log_q_fn is not None:
            ...
            for _ in range(n_batches):
                ...
                sum_kl_exact += (
                    torch.sum(self.log_prob(x_exact) - self.log_Z - log_q_x_exact).detach().cpu()
                )
...
if __name__ == "__main__":
    ...
    for i in range(n_rows):
        ...
        plot_marginal_pair(
            samples_modes, ax=axs[i, 1], bounds=plotting_bounds, marginal_dims=(i * 2, i * 2 + 1)
        )
