...
class DEMLitModule(LightningModule):
    ...
    def _log_energy_w2(self, prefix="val", energy_function=None):
        ...
        if "test" in prefix:
            ...
            generated_samples = self.generate_samples(
                self.reverse_sde, num_samples=self.eval_batch_size, resampling_interval=-1
            )
    ...
    def _log_dist_total_var(self, prefix="val", energy_function=None):
        if energy_function is None:
            energy_function = self.energy_function
        if "test" in prefix:
            data_set = energy_function.sample_test_set(self.eval_batch_size)
    ...
    def compute_log_z(self, cnf, prior, samples, prefix, name, energy_function=None):
        if energy_function is None:
            energy_function = self.energy_function
        ...
        logz = energy_function(energy_function.normalize(samples)) + nll
    ...
    def eval_step(self, prefix: str, batch: torch.Tensor, batch_idx: int) -> None:
        ...
        if batch is None:
            # TODO (alex) what is this and why do we do this it causes bugs for me.
    ...
    def eval_epoch_end(self, prefix: str):
        ...
        if self.energy_function.dimensionality == 2:
            if self.nll_with_cfm and "gen_1_cfm" in outputs:
                self.scatter_prior(prefix + "/cfm", outputs["gen_1_cfm"])
            if self.nll_with_dem and "gen_1_dem" in outputs:
        ...
        if self.nll_with_cfm:
            ...
            self.energy_function.log_on_epoch_end(
                outputs["gen_0"],
                self.energy_function(outputs["gen_0"]),
                wandb_logger,
    ...
    def on_test_epoch_end(self) -> None:
        ...
        print("Generating original temperature samples.")
        for i in range(n_batches):
            start = time.time()
            samples = self.generate_samples(
                reverse_sde=self.reverse_sde, num_samples=batch_size, resampling_interval=-1
            )
        ...
        path = f"{output_dir}/samples_temperature_{self.energy_function.temperature}_{self.num_samples_to_save}.pt"
        torch.save(final_samples, path)
        print(f"Saving samples to {path}")
        import os

        os.makedirs(self.energy_function.name, exist_ok=True)
        path2 = f"{self.energy_function.name}/samples_temperature_{self.energy_function.temperature}_{self.hparams.version}_{self.num_samples_to_save}.pt"
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.init_from_prior:
            ...
            init_states = self.generate_samples(
                reverse_sde=reverse_sde, num_samples=self.num_init_samples, resampling_interval=-1
            )
        ...
        if (
            self.hparams.input_scaling_factor is not None
            or self.hparams.output_scaling_factor is not None
        ):
