...
class ApplyRotaryEmbQKV_(torch.autograd.Function):
    ...
    @staticmethod
    def old_vmap(
        info, in_dims: Tuple[Optional[int]], *args
...
class OldApplyRotaryEmbQKV_(torch.autograd.Function):
    ...
    @staticmethod
    def vmap(
        args: List[Any], batch_size: int, _vmap_id: int = 0
    ) -> Tuple[torch.nn.Module, List[str]]:
        class VMapApplyRotaryEmbQKV(torch.nn.Module):
            def __init__(self):
                super().__init__()
        ...
        return VMapApplyRotaryEmbQKV(), ["qkv", "cos", "sin"]
    ...
    # @staticmethod
    # def setup_context(forward_fn, backward_fn, setup_args):
    # Return the same functions without modification for this example


#        return (forward_fn, backward_fn), setup_args
