...
from src.models.components.utils import sample_cat_sys, ito_logdensity, sample_birth_death_clocks
...
#     if (step < start_resampling_step) and (resampling_interval != -1):    
...
def euler_maruyama_step(
    ...
    if resampling_strategy == 'birth_death_clock':
        ...
        choice, accum_birth, accum_death, clock_thresholds, metrics \
        = sample_birth_death_clocks(x.shape[0], accum_birth, accum_death, clock_thresholds)
        x_next = x_next[choice]
        a_next = torch.zeros_like(a_next)
