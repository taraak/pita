...
class MyMLPTemperature(nn.Module):
    def __init__(
        self,
        hidden_size: int = 128,
        hidden_layers: int = 3,
        ...
        self.time_mlp = PositionalEmbedding(emb_size, time_emb)
        self.temp_mlp = PositionalEmbedding(emb_size, time_emb)
        ...
        self.self_condition = False
        concat_size = len(self.time_mlp.layer) + sum(
            map(lambda x: len(x.layer), positional_embeddings)) + len(self.temp_mlp.layer)
    ...
    def forward(self, t, x, beta):
        positional_embs = [
            self.get_submodule(f"input_mlp{i}")(x[:, i]) for i in range(x.shape[-1])
        ]
        t_emb = self.time_mlp(t.squeeze())
        beta_emb = self.temp_mlp(beta.squeeze())
        x = torch.cat((*positional_embs, t_emb, beta_emb), dim=-1)
