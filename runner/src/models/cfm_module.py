...
class CFMLitModule(LightningModule):
    ...
    def __init__(
        ...
        if ema_decay is not None:
            self.net = EMA(net(dim=self.dim), decay=ema_decay)
        else:
    ...
    def eval_step(self, batch: Any, batch_idx: int, prefix: str):
        if (prefix == "test" and self.is_image) or (self.hparams.nice_name == "short"):
...
class OneWaySF2MLitModuleV2(SF2MLitModule):
    def step(self, batch: Any, training: bool = False):
        import torchcfm
